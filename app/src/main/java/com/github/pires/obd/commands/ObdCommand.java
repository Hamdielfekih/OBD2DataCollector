...
public abstract class ObdCommand {
    ...
    protected void fillBuffer() {
        ...
        final String tempRawData = rawData.substring(1);

        while (end <= tempRawData.length()) {
            buffer.add(Integer.decode("0x" + tempRawData.substring(begin, end)));
            begin = end;
            end += 2;
        }
    }
     ...
     */
        ...
        rawByteArrayData = byteArrayOutputStream.toByteArray();
        byteArrayOutputStream.close();
    }

    private byte[] rawByteArrayData;

    public byte[] getRawByteArrayData(){
        return rawByteArrayData;
    }
